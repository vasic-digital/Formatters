...
# Formatters per CONST-035 + CONST-050(B). Submodule cascade per
# CONST-051(A). Targets $FORMATTERS_HEALTH_URL.
...
echo "=== Formatters DDoS Health-Flood Challenge ==="
echo "  url=$HEALTH_URL total=$TOTAL_REQS conc=$CONCURRENCY pass≥${MIN_PASS_PCT}%"

if [[ -z "$HEALTH_URL" ]]; then
    echo "[1/5] SKIP: FORMATTERS_HEALTH_URL unset — SKIP-OK: #env-no-target"
    echo "=== Formatters DDoS Challenge: PASSED (SKIP-OK) ==="
    exit 0
fi
...
echo
echo "=== Formatters DDoS Challenge: PASSED ==="
